...
struct PipelineBoardView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // Kanban columns. The GeometryReader claims the window's leftover
            // height and hands it to the columns explicitly — the nested
            // vertical ScrollViews have no height of their own inside a
            // horizontal ScrollView, so without this the board collapses to
            // its ideal size and floats in the middle of the window.
    ...
    private func stageColumn(_ stage: Stage) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            // Stage header — the color bar is stacked above the label row (as
            // an overlay it drew straight through the stage name).
