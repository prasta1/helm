...
struct BridgeTasksView: View {
    ...
    @State private var githubFetchErrors: Set<String> = []
    @State private var dueDateItemID: String?
    @State private var pendingDueDate = Date.now
    @State private var isParsingTask = false
    @State private var listFilter: TaskListFilter? = nil
    ...
    private func fetchGitHubItems() async {
        ...
        githubFetchErrors = result.errors
    }
            ...
            ScrollView {
                VStack(spacing: 0) {
                    ...
                    ForEach(helmLists, id: \.name) { list in
                        Button { listFilter = (listFilter == .helm) ? nil : .helm } label: {
                            listRow(name: list.name, count: list.count, color: Theme.Palette.brassDim)
                        }
                        .buttonStyle(.plain)
                        .background(
                            listFilter == .helm ? Theme.Palette.surface.opacity(0.6) : Color.clear,
                            in: RoundedRectangle(cornerRadius: 6)
                        )
                    }

                    let visibleReminderLists = reminderLists.filter { isReminderListEnabled($0.id) }
                    if !visibleReminderLists.isEmpty {
                        Divider().padding(.vertical, 8)

                        listSectionHeader("APPLE REMINDERS")
                        ForEach(visibleReminderLists) { list in
                            Button {
                                let filter = TaskListFilter.reminderList(list.id)
                                listFilter = (listFilter == filter) ? nil : filter
                            } label: {
                                listRow(
                                    name: list.title,
                            ...
                            .buttonStyle(.plain)
                            .background(
                                listFilter == .reminderList(list.id) ? Theme.Palette.surface.opacity(0.6) : Color.clear,
                                in: RoundedRectangle(cornerRadius: 6)
                            )
                        }
                    }
                    ...
                    if !settings.trackedGitHubRepos.isEmpty {
                        ...
                        ForEach(settings.trackedGitHubRepos, id: \.self) { repo in
                            Button {
                                let filter = TaskListFilter.githubRepo(repo)
                                listFilter = (listFilter == filter) ? nil : filter
                            } label: {
                                githubRepoRow(repo)
                            }
                            .buttonStyle(.plain)
                            .background(
                                listFilter == .githubRepo(repo) ? Theme.Palette.surface.opacity(0.6) : Color.clear,
                                in: RoundedRectangle(cornerRadius: 6)
                            )
                        }
                    }
    ...
    private func githubRepoRow(_ repo: String) -> some View {
        let count = githubItems.filter { $0.repoSlug == repo && !(githubOverride(for: $0.id)?.isLocallyDone ?? false) }.count
        return HStack(spacing: 10) {
            ...
            Text(repo)
                ...
                .lineLimit(1)
            Spacer()
            if githubFetchErrors.contains(repo) {
                Image(systemName: "exclamationmark.triangle")
                    .font(.system(size: 10))
                    .foregroundStyle(Theme.Palette.danger)
                    .help("Couldn't fetch \(repo) — check the repo name and your network.")
                    .accessibilityLabel("Fetch failed for \(repo)")
            } else {
                Text("\(count)")
                    .font(.system(size: 10.5, design: .monospaced))
                    .foregroundStyle(Theme.Palette.textMuted)
    ...
    private var smartInputBar: some View {
        HStack(spacing: 12) {
            ...
            TextField("Call John Friday 3pm, submit deck by EOD…", text: $newTaskText)
                .font(.system(size: 13))
                .foregroundStyle(Theme.Palette.textPrimary)
    ...
    private func addQuickTask() {
        let rawText = newTaskText.trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        if #available(macOS 26.0, iOS 26.0, *) {
            ...
            if case .available = model.availability {
                isParsingTask = true
                Task { await parseNaturalLanguageTask(rawText) }
                return
            }
        }
        saveTask(title: rawText, dueDate: nil)
    }

    /// Parses a free-form task string with Apple Intelligence and then saves it.
    @available(macOS 26.0, iOS 26.0, *)
    private func parseNaturalLanguageTask(_ rawText: String) async {
        let df = DateFormatter()
        df.dateFormat = "EEEE, MMMM d, yyyy, HH:mm"
        let nowString = df.string(from: Date.now)

        let session = LanguageModelSession()
        let prompt = "Current date and time: \(nowString). Parse the following into a task title and optional due date: \"\(rawText)\""

        do {
            let response = try await session.respond(to: prompt, generating: ParsedTask.self)
            let parsed = response.content
            let title = parsed.title.isEmpty ? rawText : parsed.title
            let dueDate = parsed.dueDate.flatMap { parseISODate($0) }
            saveTask(title: title, dueDate: dueDate)
        } catch {
            // Fall back to saving the raw text on any model error.
            saveTask(title: rawText, dueDate: nil)
        }
        isParsingTask = false
    }

    /// Saves a task to whichever destination is currently selected (Helm or Reminders).
    private func saveTask(title: String, dueDate: Date?) {
        if let listID = settings.quickAddReminderListID, service.hasFullAccess {
            ...
            modelContext.insert(Activity(kind: .task, title: title, date: dueDate ?? .now, source: .manual))
        }
    }

    /// Tries common ISO 8601 date formats that the model might produce.
    ...
    private func taskGroup(_ title: String, tasks: [TaskItem], style: DueStyle) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if tasks.isEmpty {
                ...
                VStack(spacing: 0) {
                    // Identity comes from the task's backing record (element.id),
                    // not its position — offset IDs misattribute row state and
                    // animations whenever the list is resorted or filtered.
    ...
    private func taskRow(task: TaskItem, isLast: Bool) -> some View {
        ...
        .contextMenu {
            // Every row gets right-click actions, not just GitHub items.
            Button(task.isCompleted ? "Mark as Incomplete" : "Mark as Complete") {
                toggle(task)
            }
            if let githubID = task.githubItemID {
                Button("Set Due Date…") {
                    pendingDueDate = githubOverride(for: githubID)?.dueDate ?? Calendar.current.startOfDay(for: .now)
    ...
    private var tasksDueToday: [TaskItem] {
        ...
        let completedToday = filteredDoneReminders
            .filter { $0.completionDate.map(cal.isDateInToday) ?? false }
            .map { makeItem($0, style: .time) }
        + (listFilter == nil || listFilter == .helm ? activities : [])
    ...
    private var filteredDoneReminders: [ReminderItem] {
        switch listFilter {
        case nil: return doneReminders
        case .reminderList(let id): return doneReminders.filter { $0.listID == id }
        default: return []
        }
    }
    ...
    private var logbookItems: [TaskItem] {
        ...
        let items = filteredDoneReminders.map { makeItem($0, style: .date) }
        + (listFilter == nil || listFilter == .helm ? activities : [])
            .filter { $0.kind == .task && $0.isCompleted && $0.date >= weekAgo }
            .map { makeItem($0, style: .date) }
        + (listFilter == nil ? githubItems : openGitHubItems)
    ...
    private var openHelmTasks: [Activity] {
        guard listFilter == nil || listFilter == .helm else { return [] }
        return activities.filter { $0.kind == .task && !$0.isCompleted }
    }

    private var openReminders: [ReminderItem] {
        reminders.filter {
            guard !$0.isCompleted && isReminderListEnabled($0.listID) else { return false }
            switch listFilter {
            case nil: return true
            case .reminderList(let id): return $0.listID == id
    ...
    private var openGitHubItems: [GitHubItem] {
        let open = githubItems.filter { !(githubOverride(for: $0.id)?.isLocallyDone ?? false) }
        switch listFilter {
        case nil: return open
        case .githubRepo(let slug): return open.filter { $0.repoSlug == slug }
        default: return []
        }
    }
    ...
    private var helmLists: [(name: String, count: Int)] {
        let count = activities.filter { $0.kind == .task && !$0.isCompleted }.count
        return [("Tasks", count)]
    }
}
...
enum TaskListFilter: Equatable {
    case helm
    case reminderList(String)
    case githubRepo(String)
}
...
/// Structured output for Apple Intelligence task parsing.
/// The model fills in the title (date/time stripped) and an ISO 8601 due date if one was mentioned.
@available(macOS 26.0, iOS 26.0, *)
@Generable
struct ParsedTask {
    @Guide(description: "The task title, with any date or time expressions removed. Be concise.")
    var title: String

    @Guide(description: "If a due date or time was mentioned, the datetime in ISO 8601 format (yyyy-MM-dd'T'HH:mm:ss), interpreting relative expressions like 'tomorrow' or 'next Friday' from the current date provided in the prompt. Omit this field entirely if no date or time was mentioned.")
    var dueDate: String?
}
