...
struct KeychainStore {
    ...
    func set(_ data: Data, for key: String) {
        ...
        if status != errSecSuccess {
            // A silent failure here means an API key the user just pasted never
            // persisted — at minimum leave a trace for debugging.
            print("KeychainStore: failed to save '\(key)' (OSStatus \(status))")
        }
    }
