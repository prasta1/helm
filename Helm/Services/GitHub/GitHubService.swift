...
struct GitHubService {
    ...
    /// running each request concurrently. Repos that fail to fetch (bad slug,
    /// network error, rate limit) are reported in the returned `errors` set so
    /// callers can surface a warning instead of silently showing nothing.
    func fetchItems(for repos: [String]) async -> (items: [GitHubItem], errors: Set<String>) {
        await withTaskGroup(of: (String, [GitHubItem]?).self) { group in
            for repo in repos {
                group.addTask {
                    (repo, await self.fetch(repo: repo))
                }
            }
            var all: [GitHubItem] = []
            var errors: Set<String> = []
            for await (repo, items) in group {
                if let items {
    ...
    /// Fetches open issues/PRs for a single repo. Returns nil on any network,
    /// HTTP, or decoding error so the caller can tell "no open items" apart
    /// from a failed fetch.
