...
struct RootView: View {
    ...
    @State private var deletingPipeline: Pipeline?
    ...
    var body: some View {
        #if os(macOS)
        // A floor below which the three-panel layouts stop being usable;
        // the window itself stays freely resizable above it.
        core.frame(minWidth: 960, minHeight: 600)
        #else
        core
        #endif
    }
    ...
    private var core: some View {
        ...
        .confirmationDialog(
            "Delete \u{201C}\(deletingPipeline?.name ?? "")\u{201D}?",
            isPresented: Binding(
                get: { deletingPipeline != nil },
                set: { if !$0 { deletingPipeline = nil } }
            ),
            titleVisibility: .visible
        ) {
            Button("Delete Pipeline and Deals", role: .destructive) {
                if let pipeline = deletingPipeline {
                    if selection == .pipeline(pipeline.id) { selection = .bridge }
                    modelContext.delete(pipeline)
                    try? modelContext.save()
                }
                deletingPipeline = nil
            }
        } message: {
            Text("Its stages and deals are deleted with it. This can't be undone.")
        }
        // Publishes navigation to the menu bar (Go menu, New Pipeline…) for
        // whichever window is frontmost. See HelmCommands in HelmApp.swift.
        .focusedSceneValue(\.sidebarSelection, $selection)
        .focusedSceneValue(\.newPipelineRequest, $showingNewPipeline)
    }
    ...
    private var helmSidebar: some View {
        List(selection: $selection) {
                ...
                Section("CHARTS") {
                    ForEach(pipelines) { pipeline in
                        chartLabel(pipeline, isActive: selection == .pipeline(pipeline.id))
                            ...
                            .contextMenu {
                                Button("Edit Pipeline…") { editingPipeline = pipeline }
                                Divider()
                                Button("Delete Pipeline…", role: .destructive) { deletingPipeline = pipeline }
                            }
                    }
                }
            }
            ...
            Section {
                sidebarNavLabel("Settings", icon: "gearshape", badge: nil,
    ...
    private func chartLabel(_ pipeline: Pipeline, isActive: Bool) -> some View {
        ...
        .foregroundStyle(isActive ? Theme.Palette.onNavy : Theme.Palette.sidebarText)
        .padding(.horizontal, 8)
        .padding(.vertical, 5)
        .background(
            isActive ? Theme.Palette.sidebarRowActiveBg : Color.clear,
    ...
    private var detail: some View {
        ...
        case .tasks:
            BridgeTasksView()
        case .calendar:
            CalendarView(googleClientID: settings.googleClientID)
